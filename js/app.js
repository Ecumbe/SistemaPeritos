...
document.addEventListener("DOMContentLoaded", () => {
    ...
    /* ==========================================================
       LÓGICA DEL STEPPER (v10.0 - Porcentajes Corregidos)
       ========================================================== */
    ...
    // --- Variable para saber hasta dónde ha llegado el usuario ---
    let maxStepAchieved = 1;
    
    /**
     * Actualiza la UI del stepper (los círculos y la barra)
     */
    function updateWizardStepper(stepNumber) {
        ...
        // ¡¡ARREGLO v10.0!!
        // El CSS tiene la barra gris con width: 80%
        // Los porcentajes deben ser de ese 80%
        let progressPercent = "0%";
        if (stepNumber === 2) progressPercent = "26.6%"; // (1/3) * 80%
        if (stepNumber === 3) progressPercent = "53.3%"; // (2/3) * 80%
        if (stepNumber === 4) progressPercent = "80%";   // (3/3) * 80%

        // 1. Actualizar la barra de progreso (usando la variable CSS)
        stepList.style.setProperty('--wizard-progress', progressPercent);

        // 2. Actualizar los círculos
        stepItems.forEach(item => {
            ...
            // Deshabilita clics en pasos futuros
            if (itemStep > maxStepAchieved) {
    ...
    /**
     * Función central para cambiar de paso
     */
    function navigateToStep(stepNumber) {
        // No permitir saltar a un paso no alcanzado
        if (stepNumber > maxStepAchieved) {
            console.warn(`Intento de saltar al paso ${stepNumber} (máximo alcanzado: ${maxStepAchieved})`);
            return;
        }
        ...
        // Mostrar el paso correcto y popular datos si es necesario
        if (stepNumber === 1) {
            step1Div.classList.remove('d-none');
        } else if (stepNumber === 2) {
            step2Div.classList.remove('d-none');
            popularFormularioPaso2(); // Repopular datos
        } else if (stepNumber === 3) {
            step3Div.classList.remove('d-none');
            popularFormularioPaso3(); // Repopular datos
        } else if (stepNumber === 4) {
            step4Div.classList.remove('d-none');
        }
        
        // Sincronizar la UI del stepper
        updateWizardStepper(stepNumber);
    }

    /**
     * Listener para hacer clicleable el stepper
     */
    if(wizardStepper) {
    ...
    /* ==========================================================
       FIN DE LA LÓGICA DEL STEPPER
       ========================================================== */


    // --- Lógica del PASO 1: Búsqueda ---
    ...
    // --- LÓGICA DE NAVEGACIÓN DEL WIZARD (MODIFICADA) ---
    btnContinuarPaso1.addEventListener('click', () => {
        maxStepAchieved = Math.max(maxStepAchieved, 2); // Desbloquea el paso 2
        navigateToStep(2);
        // popularFormularioPaso2(); // navigateToStep ya lo llama
    });
    ...
    document.getElementById('btn-continuar-paso3').addEventListener('click', () => {
        maxStepAchieved = Math.max(maxStepAchieved, 3); // Desbloquea el paso 3
        navigateToStep(3);
        // popularFormularioPaso3(); // navigateToStep ya lo llama
    });
    ...
    function navegarPaso4() {
        maxStepAchieved = Math.max(maxStepAchieved, 4); // Desbloquea el paso 4
        navigateToStep(4);
    }
    ...
    function resetWizard() {
        ...
        // ¡MODIFICADO! Usa la nueva función de navegación
        maxStepAchieved = 1;
        navigateToStep(1); 
    ...
    /* ==========================================================
        LÓGICA DE PESTAÑAS (¡ACTUALIZADA v9.0!)
        ========================================================== */
    const tabs = document.querySelectorAll('.app-tabs .nav-link');
    
    // --- (v8.0) Selector del Stepper (ya está definido arriba) ---
    // const wizardStepper = document.getElementById('main-wizard-stepper');
    ...
    tabs.forEach(tab => {
        tab.addEventListener('shown.bs.tab', (event) => {
            ...
            // --- (v8.0) Mostrar/Ocultar Stepper Sticky ---
            const targetId = event.target.id;
            if (targetId === 'nuevo-tab-original' || targetId === 'nuevo-tab') {
                if (wizardStepper) wizardStepper.style.display = 'block';
            } else {
    ...
    if (lastTabId) {
        ...
        if (lastTab) {
            ...
            // --- (v8.0) Mostrar/Ocultar Stepper al Cargar ---
            if (lastTabId === 'nuevo-tab-original' || lastTabId === 'nuevo-tab') {
                if (wizardStepper) wizardStepper.style.display = 'block';
            } else {
                if (wizardStepper) wizardStepper.style.display = 'none';
            }
