...
// MEJORAS: Drag & Drop (SortableJS) + Fix Ortografía en Descripciones
...
(function () {
    ...
    if (laminasContainer) {
      ...
      laminasContainer.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.btn-editar-foto') && fotoCard) {
          ...
          // Sincronizar descripción visual con dataset antes de abrir
          if (img && desc) img.dataset.description = desc.value;
          ...
          if (window.PhotoEditor && img) {
            PhotoEditor.open({
              ...
              onSave: ({ description }) => {
                ...
                // Actualizar dataset también al guardar
        ...
        if (e.target.closest('.btn-agregar-descripcion') && fotoCard) {
          ...
          if (desc) {
             ...
             if (!desc.classList.contains('d-none')) {
                 // Fix Ortografía: Forzar re-check al mostrar
                 desc.focus();
                 desc.setAttribute('spellcheck', 'false');
                 setTimeout(() => desc.setAttribute('spellcheck', 'true'), 50);
                 
                 btn.innerHTML = '<i class="fa-solid fa-check"></i> Ocultar Descripción';
             } else {
                 // Actualizar texto del botón al ocultar
                 if (desc.value.trim() !== "") {
                    btn.textContent = desc.value.substring(0, 40) + "…";
                 } else {
      ...
      // Evento Paste (Pegar imagen desde portapapeles)
      document.addEventListener('paste', (e) => {
        ...
        if (step4Marker && !step4Marker.classList.contains('active')) {
             // Opcional: Si quieres ser estricto, descomenta el return. 
             // return; 
        }
        ...
        if (!ultimaLamina) {
            // Usamos swalDark global si existe, o un fallback
            const swalOptions = window.Wizard && window.Wizard.swalDark 
                ? { customClass: window.Wizard.swalDark } 
  ...
  async function processAndDisplayImage(file, container, laminaNum) {
    ...
    // [MEJORA] Activar Sortable (Drag & Drop) en el contenedor si no está activo
    activarSortable(container);
    ...
    const desc = clone.querySelector('.foto-descripcion'); // Referencia a la descripción
    ...
    // [MEJORA] Fix Ortografía para descripciones nuevas
    if (desc) {
  ...
  /* ==========================================================
     MEJORA: FUNCIÓN PARA ACTIVAR SORTABLE (ARRASTRAR Y SOLTAR)
     Se llama automáticamente al añadir fotos
  ========================================================== */
  function activarSortable(container) {
    // Verificamos si la librería Sortable existe y si el contenedor ya fue activado
    if (typeof Sortable !== 'undefined' && !container.classList.contains('sortable-active')) {
        
        new Sortable(container, {
            animation: 150,
            ghostClass: 'bg-primary', // Clase visual del "fantasma" al arrastrar
            handle: '.card-img-top',  // Solo se arrastra desde la imagen (para no bloquear inputs)
            onEnd: function (evt) {
                // Al soltar, la lámina contenedora necesita actualizar números
        ...
        // Marcamos el contenedor para no reinicializar Sortable multiples veces
        container.classList.add('sortable-active');

        // Aseguramos que los inputs sean seleccionables sin iniciar arrastre
