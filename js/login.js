...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // tanto app.js como admin.js puedan usarla.
    ...
    loginForm.addEventListener("submit", (e) => {
        ...
        .then(data => {
            ...
            if (data.status === "success") {
                // ¡Éxito!
                Swal.fire({
                    title: `¡Bienvenido, ${data.nombreCompleto}!`,
                ...
                localStorage.setItem("sistemaPeritosUser", data.user); // El username (ej: admin, perito1)
                localStorage.setItem("sistemaPeritosFullName", data.nombreCompleto); // El nombre

                // --- [INICIO DE LA MODIFICIÓN: DESVIADOR DE ROLES] ---
                // Redirigimos a la página principal de la app
                setTimeout(() => {
                    if (data.user === "admin") {
                        // Si el usuario es "admin", va al panel de administrador
                        window.location.href = "admin.html";
                    } else {
                        // Cualquier otro usuario va a la app normal
