// js/wizard.js
// Lógica del Wizard (Pasos 1–4) + SunEditor + Autosave (Debounce) + Generación de documentos
...
(function () {
  ...
  function initWizard(options) {
    ...
    // Chequea si hay borrador (autosave en la nube)
    checkForCloudDraft();
  }
/* ==========================================================
   SUNEDITOR (Rich Text) - CON ESPERA AUTOMÁTICA
========================================================== */
function initRichTextEditors() {
  const Sun = window.SUNEDITOR || window.SunEditor;
    // 1. VERIFICACIÓN DE SEGURIDAD Y REINTENTO
    // Si SunEditor no existe, esperamos un poco y reintentamos (hasta 10 veces)
    if (typeof Sun === 'undefined') {
        if (!window.sunEditorRetries) window.sunEditorRetries = 0;
        window.sunEditorRetries++;
        
        if (window.sunEditorRetries < 20) { // Intentar durante 2 segundos aprox
            console.warn(`⏳ SunEditor aún no carga. Reintentando (${window.sunEditorRetries})...`);
            setTimeout(initRichTextEditors, 100); // Volver a llamar a esta función en 100ms
            return;
        } else {
            console.error("⚠️ Error Crítico: SunEditor no cargó después de varios intentos. Se usarán textareas simples.");
            return; // Nos rendimos, la app sigue con textareas normales
        }
    }
    ...
    const commonConfig = {
        ...
        mode: 'classic',
        // Usamos window.SUNEDITOR_LANG para mayor seguridad
...
// --- EL TRUCO PARA ACTIVAR ORTOGRAFÍA EN SUNEDITOR ---
    ...
    // 3. CREACIÓN
    try {
        // Función auxiliar para crear si existe el elemento
        const createIfExist = (id, config) => {
            const el = document.getElementById(id);
            if (el && el.tagName === 'TEXTAREA' && !el.style.display.includes('none')) {
                // Verificamos display none para no reinicializar uno ya creado
        ...
        // Asignamos a las variables globales definidas arriba en tu archivo
  ...
  /* ==========================================================
     AUTOSAVE (Borrador en la nube) - CON DEBOUNCE
  ========================================================== */
  ...
  // Llamar al servidor para borrar borrador en la nube
  API.borrarBorrador(loggedInUser)
    .then(() => {
      console.log("Borrador eliminado en la nube.");
    })
  ...
  function inicializarBotonesNavegacion() {
    ...
    if (btnCont4) btnCont4.addEventListener('click', () => {
      // Comprobar contenido usando el editor si existe, o el value si no
    ...
    generators.forEach(gen => {
      ...
      if (btn) {
        btn.addEventListener('click', () => {
          ...
          // Lógica para insertar en SunEditor si existe, o en textarea normal
  ...
  function gatherAllTextData() {
    return {
      ...
      datosCuerpo: {
        ...
        // AQUÍ USAMOS GETCONTENTS() PARA OBTENER EL HTML DEL EDITOR
  ...
  window.Wizard = {
    ...
    swalDark // Exponemos swalDark para que fotos.js pueda usarlo si lo necesita
  };
