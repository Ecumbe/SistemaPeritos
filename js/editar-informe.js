...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- VARIABLES GLOBALES PARA SUNEDITOR ---
    ...
    /* ==========================================================
       LÓGICA DE INICIALIZACIÓN DE SUNEDITOR (Igual que Wizard.js)
       ========================================================== */
    function initRichTextEditors() {
        const Sun = window.SUNEDITOR;
        if (typeof Sun === 'undefined') {
            console.warn("SunEditor no cargó, reintentando en 100ms...");
        ...
        // Función auxiliar para activar corrección ortográfica
        const enableSpellCheck = (editor) => {
            if (editor && editor.core && editor.core.context && editor.core.context.element.wysiwyg) {
                editor.core.context.element.wysiwyg.setAttribute('spellcheck', 'true');
                editor.core.context.element.wysiwyg.setAttribute('lang', 'es');
            }
        };

        try {
            // Inicializar solo si existen los elementos
            if (document.getElementById('informe-referencia')) {
                editorReferencia = Sun.create('informe-referencia', commonConfig);
                editorReferencia.onload = () => enableSpellCheck(editorReferencia);
            }
            
            if (document.getElementById('informe-objeto')) {
                editorObjeto = Sun.create('informe-objeto', { ...commonConfig, minHeight: '100px' });
                editorObjeto.onload = () => enableSpellCheck(editorObjeto);
            }
            
            if (document.getElementById('informe-reconocimiento')) {
                editorReconocimiento = Sun.create('informe-reconocimiento', { ...commonConfig, minHeight: '300px' });
                editorReconocimiento.onload = () => enableSpellCheck(editorReconocimiento);
            }
            
            if (document.getElementById('informe-conclusiones')) {
                editorConclusiones = Sun.create('informe-conclusiones', { ...commonConfig, minHeight: '150px' });
                editorConclusiones.onload = () => enableSpellCheck(editorConclusiones);
            }

            // Agregar listener para detectar cambios y mostrar botones de guardado
            [editorReferencia, editorObjeto, editorReconocimiento, editorConclusiones].forEach(ed => {
                if(ed) ed.onChange = () => showNavButtons();
            });
    ...
    document.getElementById('btn-continuar-paso4').addEventListener('click', () => {
        // Validar contenido del editor de reconocimiento
        const recoContent = editorReconocimiento ? editorReconocimiento.getContents() : document.getElementById('informe-reconocimiento').value;
        
        if (!recoContent || !recoContent.trim()) {
             Swal.fire({
                icon: 'warning', title: 'Campo Vacío', text: 'El reconocimiento está vacío. ¿Continuar?',
                showCancelButton: true, confirmButtonText: 'Sí', customClass: swalDark
            }).then((result) => {
                if (result.isConfirmed) irPaso4();
            });
        } else {
            irPaso4();
        }
    ...
    function popularFormularioPaso3(data) {
        ...
        // [MODIFICADO] Cargar datos en SunEditor si existe, si no, en textarea normal
        if(editorReferencia) editorReferencia.setContents(data.Cuerpo_Referencia);
        else document.getElementById('informe-referencia').value = data.Cuerpo_Referencia;

        if(editorObjeto) editorObjeto.setContents(data.Cuerpo_Objeto);
        else document.getElementById('informe-objeto').value = data.Cuerpo_Objeto;
        
        if(editorReconocimiento) editorReconocimiento.setContents(data.Cuerpo_Reconocimiento);
        else document.getElementById('informe-reconocimiento').value = data.Cuerpo_Reconocimiento;

        if(editorConclusiones) editorConclusiones.setContents(data.Cuerpo_Conclusiones);
        else document.getElementById('informe-conclusiones').value = data.Cuerpo_Conclusiones;

        // Listeners para textareas normales (las que no tienen editor)
    ...
    function popularFormularioPaso4(fotos) {
        ...
        for (const laminaNum of laminasOrdenadas) {
            ...
            // Activar Sortable en este contenedor cargado
            activarSortable(fotosContainer);
    ...
    // --- 5. LÓGICA DE ACTUALIZACIÓN/CLONACIÓN ---
    ...
    document.getElementById('btn-actualizar-flagrancia').addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                Swal.fire({ icon: 'success', title: 'Datos Precargados', text: 'Los campos han sido actualizados.', customClass: swalDark });
            } else {
    ...
    // --- 6. FUNCIONES DE AYUDA (Generadores actualizados para usar Editor) ---
    ...
    document.getElementById('btn-generar-referencia').addEventListener('click', () => {
        ...
        if (editorReferencia) editorReferencia.setContents(texto);
        else document.getElementById('informe-referencia').value = texto;
    });
    ...
    document.getElementById('btn-generar-objeto').addEventListener('click', () => {
        ...
        if (editorObjeto) editorObjeto.setContents(texto);
        else document.getElementById('informe-objeto').value = texto;
    });
    ...
    document.getElementById('btn-generar-conclusion').addEventListener('click', () => {
        ...
        if (editorConclusiones) editorConclusiones.setContents(texto);
        else document.getElementById('informe-conclusiones').value = texto;
    });
    ...
    /* ==========================================================
       7. LÓGICA DE FOTOS (Reutilización de código)
       ========================================================== */
    const laminasContainer = document.getElementById('laminas-container');
    const templateLamina = document.getElementById('template-lamina');
    const templateFoto = document.getElementById('template-foto');
    ...
    laminasContainer.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.btn-editar-foto')) {
            ...
            // Sincronizar dataset antes de abrir
            img.dataset.description = descripcion.value; 
        ...
        if (e.target.closest('.btn-agregar-descripcion')) {
            ...
            if (!descripcion.classList.contains('d-none')) {
                ...
                descripcion.setAttribute('spellcheck', 'true');
                descripcion.setAttribute('lang', 'es');
    ...
    function activarSortable(container) {
        if (typeof Sortable !== 'undefined' && !container.classList.contains('sortable-active')) {
            new Sortable(container, {
                animation: 150,
                ghostClass: 'bg-primary',
                handle: '.card-img-top',
                onEnd: function (evt) {
                    const laminaCard = container.closest('.lamina-card');
                    updatePhotoNumbers(laminaCard);
                    showNavButtons();
                }
            });
    ...
    document.addEventListener('paste', (e) => {
        ...
        if (step4Div.classList.contains('d-none')) return;
        
        const ultimaLamina = laminasContainer.querySelector('.lamina-card:last-of-type');
        if (!ultimaLamina) {
            Swal.fire({ icon: 'warning', title: 'No hay lámina', text: 'Agregue una lámina primero.', customClass: swalDark });
            return;
        }
    ...
    /* ==========================================================
       8. LÓGICA DE GUARDADO (Actualizada para leer SunEditor)
       ========================================================== */
    ...
    function gatherAllTextData() {
        ...
        const datosCuerpo = {
            ...
            // [MODIFICADO] Usar .getContents() del editor o .value del textarea
            referencia: editorReferencia ? editorReferencia.getContents() : document.getElementById('informe-referencia').value,
            objeto: editorObjeto ? editorObjeto.getContents() : document.getElementById('informe-objeto').value,
            reconocimiento: editorReconocimiento ? editorReconocimiento.getContents() : document.getElementById('informe-reconocimiento').value,
            conclusiones: editorConclusiones ? editorConclusiones.getContents() : document.getElementById('informe-conclusiones').value
        };
...
// --- Performance helper: ensure dynamic <img> use lazy/async without touching your logic ---
document.addEventListener("DOMContentLoaded", () => {
  const setImgPerfAttrs = (img) => {
    try {
  ...
  document.querySelectorAll("img").forEach(setImgPerfAttrs);
  ...
  const obs = new MutationObserver((mutations) => {
    for (const m of mutations) {
      for (const node of m.addedNodes) {
        ...
        if (node.tagName === "IMG") {
          setImgPerfAttrs(node);
        } else if (node.querySelectorAll) {
          node.querySelectorAll("img").forEach(setImgPerfAttrs);
        }
      }
    }
  });
